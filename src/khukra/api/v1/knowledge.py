...
from khukra.api.schemas import KnowledgeAssetCreate, KnowledgeAssetInfo, SavedQueryCreate, SavedQueryInfo
...
@router.get("/assets", response_model=list[KnowledgeAssetInfo])
def list_assets(
...
@router.post("/assets", response_model=KnowledgeAssetInfo)
def create_asset(
...
@router.get("/queries", response_model=list[SavedQueryInfo])
def list_queries(domain: str | None = None, product_id: str | None = None) -> list[SavedQueryInfo]:
    return [
        SavedQueryInfo(**q)
        for q in get_app_container().knowledge.list_queries(domain=domain, product_id=product_id)
    ]


@router.post("/queries", response_model=SavedQueryInfo)
def save_query(body: SavedQueryCreate, user: dict = Depends(require_user)) -> SavedQueryInfo:
