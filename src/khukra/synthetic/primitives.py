"""Reusable stochastic and systems primitives for synthetic research data."""
...
def jump_diffusion(
    ...
    """Merton-style jump diffusion for stochastic shocks and discontinuities."""
    y = np.zeros(n)
    y[0] = initial
    for t in range(1, n):
...
def stochastic_volatility(
    ...
    """Mean-reverting variance process, clipped for numerical stability."""
    variance = np.zeros(n)
    variance[0] = max(long_run_var, 1e-6)
    for t in range(1, n):
        shock = vol_of_vol * np.sqrt(max(variance[t - 1], 1e-6)) * rng.normal()
        variance[t] = variance[t - 1] + reversion * (long_run_var - variance[t - 1]) + shock
...
def compound_poisson_shocks(
    ...
    """Compound Poisson shock series for rare severe events."""
...
def bayesian_belief_state(
    observations: np.ndarray,
    process_noise: float,
    measurement_noise: float,
    rng: np.random.Generator,
) -> np.ndarray:
    """Noisy latent belief state used by intelligence/computing models."""
    belief = np.zeros(len(observations))
    belief[0] = observations[0] + rng.normal(0, measurement_noise)
    gain = process_noise / max(process_noise + measurement_noise, 1e-6)
    for t in range(1, len(observations)):
        prediction = belief[t - 1] + rng.normal(0, process_noise)
        belief[t] = prediction + gain * (observations[t] - prediction)
    return belief
...
def queueing_load(
    ...
    for t in range(n):
        ...
        service = min(backlog + arrivals, service_rate)
        backlog = max(0.0, backlog + arrivals - service)
        load[t] = backlog
    return load
...
def forecast_holt_linear(
    ...
    """Simple Holt linear trend forecast with residual-based intervals."""
    n = len(y)
    level = y[0]
