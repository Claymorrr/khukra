from typing import Any

import numpy as np
from scipy.integrate import solve_ivp

from khukra.core.model import Model, ModelResult
from khukra.domains.physical.analytics import mechanical_energy, settling_time
from khukra.domains.physical.core import SolverResultSummary
from khukra.domains.physical.models_registry import SOLVER_SPECS
from khukra.domains.physical.solver_base import enrich_solver_result, prepare_solver_parameters


class DampedOscillator(Model):
    """Mass-spring-damper: m x'' + c x' + k x = 0."""

    domain = "physical"
    subdomain = "mechanics"
    name = "damped_oscillator"

    def default_parameters(self) -> dict[str, Any]:
        return {
            "mass": 1.0,
            "damping": 0.5,
            "stiffness": 4.0,
            "initial_displacement": 1.0,
            "initial_velocity": 0.0,
            "duration": 10.0,
            "dt": 0.05,
        }

    def run(self, parameters: dict[str, Any] | None = None) -> ModelResult:
        p = prepare_solver_parameters(self, parameters)
        m, c, k = p["mass"], p["damping"], p["stiffness"]

        def dynamics(_t, state):
            x, v = state
            return [v, (-c * v - k * x) / m]

        t_eval = np.arange(0, p["duration"] + p["dt"], p["dt"])
        sol = solve_ivp(
            dynamics,
            (0, p["duration"]),
            [p["initial_displacement"], p["initial_velocity"]],
            t_eval=t_eval,
            method="RK45",
        )

        displacement = sol.y[0]
        velocity = sol.y[1]
        energy = mechanical_energy(m, velocity, k, displacement)

        result = ModelResult(
            domain=self.domain,
            subdomain=self.subdomain,
            model_name=self.name,
            parameters=p,
            metrics={
                "peak_displacement": float(np.max(np.abs(displacement))),
                "final_energy": float(energy[-1]),
                "settling_time": settling_time(sol.t, displacement),
            },
            series={
                "time": sol.t.tolist(),
                "displacement": displacement.tolist(),
                "velocity": velocity.tolist(),
                "energy": energy.tolist(),
            },
        )
        spec = SOLVER_SPECS[self.name]
        numerical = SolverResultSummary(
            integration_success=bool(sol.success),
            n_steps=int(sol.t.size),
            notes=str(sol.message) if sol.message else "",
        )
        return enrich_solver_result(result, spec, numerical=numerical)
